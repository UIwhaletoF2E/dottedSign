$(document).ready(function() {
	...
	$('#select-file-text').click(function(e) {
	    ...
	    $('input#get_file').change(function(e){
	        ...
	        $("#update-name-pdf").append("<img src='img/icon-img.svg' alt='img'> <b>已選擇圖片</b>"
	        	+ imgName + "<button id='upload-submit'><img src='img/icon-upload.svg' alt='upload'></button>");
	    });
	});
	$('#select-pdf-text').click(function(e) {
	    ...
	    $('input#get_pdf').change(function(e){
	        ...
	        $("#update-name-pdf").append("<img src='img/icon-pdf.svg' alt='pdf'> <b>已選擇檔案</b>"
	        	+ pdfName + "<button id='upload-submit'><img src='img/icon-upload.svg' alt='upload'></button>");
	    });
	});
	        ...
	        for(var i = 0; i < droppedFiles.length; i++)
	        {
	        	var filesType = droppedFiles[i].type.substring(12);
	          	...
	          	if (droppedFiles[i].type.search("pdf")){
	          		$("#update-name-pdf").append("<img src='img/icon-pdf.svg' alt='pdf'> <b>已選擇檔案</b>"
	          			+ droppedFiles[i].name+
	          			"<button id='upload-submit'><img src='img/icon-upload.svg' alt='upload'></button>");
